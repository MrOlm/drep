...
#import drep_modules.d_filter as d_filter
...
def d_choose_wrapper(wd,**kwargs):
    ...
    # If Chdb doesn't exist, make it
        ...
        Chdb = d_filter.run_checkM_wrapper(bdb, workDirectory, **kwargs)
...
def pick_winners(Sdb, Cdb):
    ...
    for cluster in Cdb['ANIn_cluster'].unique():
        d = Sdb[Sdb['genome'].isin(Cdb['genome'][Cdb['ANIn_cluster'] == cluster].tolist())]
        score = d['score'].max()
