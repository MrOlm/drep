...
def test_plot_6_1(self):
    ...
    assert len([os.path.basename(f) for f in glob.glob(fig_dir + '*')]) == 0

    drep.d_analyze.plot_winners_from_wd(wd, plot_dir=fig_dir)
    ...
    db_loc = os.path.join(wd_loc, 'data_tables', 'Widb.csv')
    os.remove(db_loc)
    for f in glob.glob(fig_dir + '*'):
        os.remove(f)

    #drep.d_analyze.plot_winners_from_wd(wd, plot_dir=fig_dir)
    drep.d_analyze.d_analyze_wrapper(wd_loc, plots=['6'])
...
def test_plotting_functional_1(self):
    '''
...
def test_plotting_functional_2(self):
    '''
